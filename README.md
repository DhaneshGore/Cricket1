...
# Cricket File Merger Application

This application allows you to select and merge multiple `.txt` files for different cricket objects (ball, bat, umpire, bowler, batsman, fielder, wicketkeeper, stumps) into a single output file using a graphical file dialog.
...
1. **Navigate to the Project Directory**
   ...
   Open your terminal or command prompt and navigate to the directory containing the `Cricket/cricket.py` file:
...
3. **Select Files for Each Object**
   
   - For each cricket object (ball, bat, umpire, bowler, batsman, fielder, wicketkeeper, stumps), a file dialog will appear.
   - Select one or more `.txt` files for each object and click "Open".
   - If you do not want to select files for an object, simply close the dialog or click "Cancel".

4. **Choose Save Location**
   
   - After selecting files for all objects, a save dialog will appear.
   - Choose the location and name for the merged output `.txt` file and click "Save".

5. **Completion**
   
   - The application will merge the contents of all selected files and save them to the specified output file.
   - You will see a message in the terminal indicating the location of the saved file.

## Notes

- Each selected file's content will be separated by a new line in the merged file.
- If you do not select any files for an object, it will be skipped.
- If you cancel the save dialog, the application will exit without saving.

## Example

```
Select .txt files for ball:
Select .txt files for bat:
...
All object files selected. Now processing...
Final merged file saved at: C:/path/to/your/merged_file.txt
```
