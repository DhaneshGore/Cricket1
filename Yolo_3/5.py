...
root = Tk()
root.withdraw()
video_path = filedialog.askopenfilename(title="Select a video file", filetypes=[("Video Files", "*.mp4 *.mov *.avi")])
if not video_path:
...
while cap.isOpened():
    ...
    if batsman_detected:
        ...
        for box in shot_results.boxes:
            ...
            # === Flip and run hypothetical detection ===
            ...
            break  # Only first shot is considered

    draw_two_bar_chart(frame, actual_cls, hypo_cls, actual_conf, hypo_conf)

    out.write(frame)
    cv2.imshow("Shot Style Vision", frame)
